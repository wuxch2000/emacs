...
(defun wuxch-ibuffer-mode-hook ()
  ...
  (define-key ibuffer-mode-map [(meta up)] 'ibuffer-backward-filter-group)
  (define-key ibuffer-mode-map [(meta down)] 'ibuffer-forward-filter-group)
  ...
  (define-key ibuffer-mode-map (kbd "/") 'ignore)
  (define-key ibuffer-mode-map (kbd "/") (lambda ()
                                           (interactive)
                                           (goto-line 3)
...
;; (add-hook 'kill-buffer-hook 'wuxch-kill-buffer-hook)

;; (defun wuxch-kill-buffer-hook ()
;; (if (and (buffer-modified-p)
;;          (not (buffer-file-name))
;;          (not (string= (buffer-name) "*info*"))
;;          (not (string= (buffer-name) "*Help*"))
;;          )
;;     (if (not (y-or-n-p (concat "buffer modified, quit?")))
;;         (error "quit without saved")
;;       )
;;   )
;; )
...
(defun wuxch-get-buffer-file-ext (&optional buf)
  "wuxch-get-buffer-file-ext:"
  (unless buf
    (setq buf (current-buffer)))
  (if (buffer-file-name buf)
      (file-name-extension (file-name-nondirectory (buffer-file-name buf)))
    nil
    )
  )
