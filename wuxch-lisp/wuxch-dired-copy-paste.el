...
(defvar dired-copied-cutted-files-pool nil "global variable to store copied or cut files")
...
(defun wuxch-dired-do-copy-cut(is-copy)
  ...
  (let ((copy-cut-string)(num (safe-length dired-copied-cutted-files-pool)))
    ...
    (if (eq num 1)
        (progn
          (message "\"%s\" is %s"
                   (file-name-nondirectory (car dired-copied-cutted-files-pool))
                   copy-cut-string)
          )
      (progn
      ...
      (let ((copy-cut-string)
        ...
        (dolist (src-file dired-copied-cutted-files-pool)
          (let ((dst-file))
            ...
            ;; when only 1 file is be copied or cut, maybe I want to change a file name.
            (if (eq file-number 1)
                (progn
                  (let ((original-dst-file (file-name-nondirectory src-file)))
                    (setq dst-file
                          (read-string (concat "input new file name[ENTER:\"" original-dst-file "\"]:")
                                       nil nil original-dst-file))
                    (setq dst-file (concat (dired-current-directory) dst-file))
                    )
                  )
              (progn
