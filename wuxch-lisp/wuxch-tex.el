...
;;;   (define-key outline-minor-mode-map [(control right)] 'outline-demote)
  ...
  (local-set-key [(control x)(i)]   'wuxch-tex-indent)
  (local-set-key [(control x)(n)(f)]   'beamer-narrow-to-frame)

  (local-set-key [(control c)(b)]   'ignore)
  (local-set-key [(control c)(b)]   'wuxch-latex-mark-begin-end)


  ;; (local-set-key [(control c)(t)]   'ignore)
  ;; (local-set-key [(control c)(t)]   'wuxch-pgf-tikz-keyword-setup)
...
(defun wuxch-pstrick-keyword-setyp ()
  "wuxch-pstrick-keyword-setyp:"
...
(defun wuxch-pgf-tikz-keyword-setup ()
  "wuxch-pgf-tikz-keyword-setup:"
  (interactive)
  (font-lock-add-keywords
   nil
   '(
     ("\\(\\\\node\\|\\\\tikzstyle\\|\\\\draw\\|\\\\filldraw\\|\\\\coordinate\\)"
      0 'font-lock-builtin-face t)
     ("\\<\\(at\\|right=of\\|left=of\\|down=of\\|up=of\\)\\>" 0 'font-lock-keyword-face t)
     ("[[:blank:]]+(\\([[:alpha:]][[:alnum:]]*\\))\\([[:blank:]]+\\|;+\\)" 1 'font-lock-variable-name-face t)
     ;; ("\\(\\\\tikzstyle\\)" 0 'font-lock-function-name-face t)
     )
   )
  (font-lock-mode nil)
  (font-lock-mode t)
...
(defconst wuxch-tex-file-type-tikz "tikzpicture")
...
(defun wuxch-latex-set-tex-file-type ()
  ...
  (let ((pos (point))
    ...
    (when (re-search-forward "\\documentclass.*{\\([[:alpha:]]+\\)}" nil t)
      ...
      (unless (string= file-type "beamer")
        (goto-char (point-min))
        (when (re-search-forward "\\\\begin{tikzpicture}" nil t)
          (setq file-type wuxch-tex-file-type-tikz)
          )
        )
      )
...
(defun wuxch-latex-mark-begin-end ()
  "wuxch-latex-mark-begin-end:"
  (interactive)
  (interactive)
  (let ((pos (point))
        (key))
    (when (re-search-backward "\\\\begin{\\(.*\\)}")
      (setq key (match-string 1))
      (cua-set-mark)
      (if (re-search-forward (concat "\\\\end{" key "}"))
          (progn
            (message "frame marked")
            )
        (progn
...
(defun wuxch-tex-indent ()
  (interactive)
  (let ((pos (point)))
    (when (string= "beamer" (buffer-local-value 'wuxch-tex-file-type (current-buffer)))
      (wuxch-tex-indent-beamer)
      )
    (when (string= wuxch-tex-file-type-tikz (buffer-local-value 'wuxch-tex-file-type (current-buffer)))
      (wuxch-tex-indent-tikz)
      )
    (when (string= "sty" (downcase (file-name-extension (buffer-file-name))) )
      (wuxch-tex-indent-sty)
      )
    (goto-char pos))
  )

(defun wuxch-tex-indent-sty ()
  (let ((beg-pos)(end-pos))
    (when (re-search-backward "\\(re\\)?newcommand{.*}\\(\\[[0-9]\\]\\)?")
      (setq beg-pos (point))
...
(defun wuxch-tex-indent-tikz ()
  (let ((beg-pos))
    (when (re-search-backward "\\\\begin{tikzpicture}")
      (setq beg-pos (point))
      (when (re-search-forward "\\\\end{tikzpicture}")
        (indent-region beg-pos (point))
        )))
  )

(defun wuxch-tex-indent-beamer ()
  (let ((beg-pos)(end-pos))
    (when (re-search-backward "\\\\begin{frame}")
      (setq beg-pos (point))
      (when (re-search-forward "\\\\end{frame}")
        (indent-region beg-pos (point))
        )))
  )
...
(defun wuxch-view-pdf ()
  ...
  (let* ((tex-file (concat (TeX-master-file nil nil nil) ".tex"))
    ...
    (setq cmd (concat cmd " -x \"pdfopen --rx --file %{output} --page %{page+1}\""))
    ;; (setq cmd (concat cmd " -x \"wuxch_pdfopen.bat %{page+1} %{output} \""))
...
(defun quit-and-delete-overlay (ov)
  "quit-and-delete-overlay:"
  (when (overlayp ov)
    (delete-overlay ov)
    )
  (signal 'quit nil)
...
(defun customize-search-and-replace (search-for replace-with)
  ...
  (let ((current-pos (point))
    ...
    (while (re-search-forward search-for nil t)
      (when (match-string 0)
        ...
        (condition-case  nil
            (progn
              (setq ret (y-or-n-ex (concat "update [" (match-string 0) "] ?")))
              )
          ('quit
           (quit-and-delete-overlay ov)
           )
          )
        (cond
         ((eq ret 'act)
...
(defun tex-number-to-item ()
  "tex-number-to-item"
  (interactive)
  (customize-search-and-replace "^[[:space:]]*[(（]?[[:digit:]][)）]?[、]?[\\.]?[[:space:]]*" "  \\\\item ")
  )
