...
(define-key ctl-x-4-map "l" 'wuxch-bookmark-bmenu-list-other-windows)
...
(defun my-bookmark-bmenu-mode-hook-fun ()
  ...
  (define-key bookmark-bmenu-mode-map [(/)] 'ignore)
  (define-key bookmark-bmenu-mode-map [(/)] 'wuxch-bookmark-search)

  )

(defun wuxch-bookmark-search ()
  (interactive)
  (goto-char (point-min))
  (forward-line 2)
      ...
      (while (< sequence 10)
        (if (< (+ sequence wuxch-bookmark-first-line-of-insert) (+ temp-max-line 1))
            (progn
              ...
              (overlay-put (make-overlay over-lay-pos-start over-lay-pos-end)
...
(defconst bookmark-list-buffer-name "*Bookmark List*")
(defun wuxch-bookmark-bmenu-list ()
  ""
  (interactive)
  (bookmark-bmenu-list)
  (if (interactive-p)
      (switch-to-buffer (get-buffer-create bookmark-list-buffer-name))
    (set-buffer (get-buffer-create bookmark-list-buffer-name)))
  (my-bookmark-add-sequence)
  (hl-line-mode t)
  )

(defun wuxch-bookmark-bmenu-list-other-windows ()
  ""
  (interactive)
...
(defun wuxch-bookmark-bmenu-this-window ()
  ...
  (kill-buffer bookmark-list-buffer-name)
  )
